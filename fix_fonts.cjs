...
let contentNS = fs.readFileSync('src/components/NarrativeStream.tsx', 'utf8');

contentNS = contentNS.replace(/scriptureFontSizeSetting === 'xs' \? 'text-\[11px\]' : scriptureFontSizeSetting === 'sm' \? 'text-\[13\.5px\]' : scriptureFontSizeSetting === 'base' \? 'text-\[15\.5px\]' : 'text-\[18px\]'/g, 
"scriptureFontSizeSetting === 'xs' ? 'text-[14px]' : scriptureFontSizeSetting === 'sm' ? 'text-[16px]' : scriptureFontSizeSetting === 'base' ? 'text-[18px]' : 'text-[20px]'");

fs.writeFileSync('src/components/NarrativeStream.tsx', contentNS);
